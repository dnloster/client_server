...
import { getSignCookie } from "~/App/common/authService";
import { API_ENDPOINT, STRING } from "~/App/apis/index";
...
class Navigation extends Component {
    ...
    fetch = async () => {
        const { imageAllActions } = this.props;
        const { fetchLogoRequest } = imageAllActions;
        await fetchLogoRequest();
    };
    ...
    componentWillMount() {
        ...
        this.setState({
            logo,
            size: window.innerWidth > 557.98 ? "large" : "small",
        });
    }
        ...
        return (
            ...
            >
                ...
                <Link to="/">
                    <img
                        // src="/logo192.png"
                        src={API_ENDPOINT + this.props.logo}
                        alt="Tours"
                        ...
                        {showCart && (
                            <li className="nav-item ht-nav-item">
                                <Link
                                    ...
                                    className="nav-link ht-nav-link"
                                >
                                    <i className="fas fa-shopping-cart"></i> Đơn
                                    hàng
                                </Link>
                            </li>
                        )}
...
const mapDispatchToProps = (dispatch) => {
    return {
        imageAllActions: bindActionCreators(imageAction, dispatch),
    };
};

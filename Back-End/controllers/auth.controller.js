            ...
            bcrypt
                ...
                .then(async (passwordHash) => {
                    ...
                    const html = `Hi there,
                        ...
                        "verifyToken",
                        "dnloster"
                    )}=${jwt.sign(verifyToken, "dnloster")}</a>
                        <br/><br/>
                        Have a pleasant day.`;
    ...
    Accounts.getByEmailAndRole(email, "user")
        .then(async (account) => {
            ...
            const html = `Hi there,
                ...
                account.email,
                "dnloster"
            )}&wXvkihdDAD9D8FI9Nwpf=${jwt.sign(Date.now(), "dnloster")}">${
                process.env.FRONT_END
            }forgotPassword?${jwt.sign("verifyToken", "dnloster")}=${jwt.sign(
                verifyToken,
                "dnloster"
            )}</a>
                        <br/><br/>
...
exports.forgotPasswordStep2 = (req, res, next) => {
    ...
    try {
        ...
        const date = jwt.verify(req.query.wXvkihdDAD9D8FI9Nwpf, "dnloster");
        const email = jwt.verify(req.query.QZmWYU22y2zb2qZg8clJ, "dnloster");
            ...
            await Accounts.getByEmailAndRole(googleUser.email, req.body.role)
                .then(async (account) => {
                    if (account) {
                        ...
                        const loadAccount = await Accounts.getByIdGoogle(
                            googleUser.id
                        );

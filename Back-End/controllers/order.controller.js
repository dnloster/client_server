...
const CLIENT_ID =
    "AVzJuf5NEz2NtZA_rAmWefMMYeT0LB6ew2dLZBxc95eaGrkh12as2-9tjiEORXOpLOupFeJnpyprcCrh";

const APP_SECRET =
    "EBzUDQxwSqx19x5oL3AZ3J_T1MFx-SVvKOb3dDOH69H41p_NmczUZq-eQI8FyefLRKIW9b0NBK3QP6XQ";
    ...
    Order.createOrder(newOrder, function (err, order) {
        ...
        let secureCode = MD5(
            `${process.env.MERCHANT_SITE_CODE} ${process.env.RETURN_URL} ${
                ...
                newOrder.totalPrice
            } vnd 1 0 0 0 0 Tour:${tour.titleTour
                .trim()
                .replace(/\s/g, "")}People-${newOrder.numberPeople}Children-${
                newOrder.numberChildren
            } ${newOrder.buyer.trim().replace(/\s/g, "")}*|*${newOrder.email
                .trim()
...
exports.capturePaymentPayPal = async (req, res) => {
    ...
    const data = await response.json(orderId);
    // console.log(data);
    res.json(data);
    // return data;
};
